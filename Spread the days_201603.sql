 -----------------------------------------analyse runs ----------------------------------------------------------------------------------------------------------
begin 
dw_bib_utl. PERIOD_BATCH_ANALYSIS(v_task_like =>'%CS6_TE_S2C_FCT_CDR_PREPAID_RATED_MA%',v_from_key =>20171024, v_to_key=>20171024);
...
update dw_rt_runs 
set start_date=to_date(DIY_PARAMETER,'YYYYMMDD') - 30 ,
 rows_loaded=null  -- if you want them to runs as FIRST
where run_status='RUNABLE'
and task like 'CS5_TE_S2C_FCT_CDR_ADJUSTMENT_IBF_MA'
and diy_parameter = 20170731
--and start_date   < sysdate - 4
--<>to_date(DIY_PARAMETER,'YYYYMMDD')
;
