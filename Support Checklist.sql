...
WHERE PARTITIONING_KEY in ('Date','Batch')
) loop

insert into gt_tasks(task, task_type, table_name, template_name, dw_task_Id)
select nvL(tablespace_Name,'NO TABLESPACE'), partition_Name, table_Name, table_owner, 1
from 
(
SELECT *
FROM ALL_TAB_PARTITIONS p
where table_name = i.table_name
and table_owner = i.schema_name
...
select 
s.*,
case when current_ts <> ts_var then 'INCORRECT TS!' else null end ts_status,
case when partition_name not like 'P_%' then 'INCORRECT PARTNAME!' else null end part_status
from
(
select
b.owner,
b.table_name,
b.partition_name,
case when (b.tablespace like '%_M_%' and b.tablespace not like '%META%') then substr(b.tablespace, 1, length(b.tablespace) - 7) else b.tablespace end current_ts, 
case when (m.tablespace_variable like '%_M_%' and m.tablespace_variable not like '%META%') then substr(m.TABLESPACE_VARIABLE, 1, length(m.TABLESPACE_VARIABLE) - 9) else m.tablespace_variable end TS_VAR, 
--b.ts_year_month,
--b.current_year_month curr_year_month,
m.TABLESPACE_VARIABLE
from 
(
select a.*,
nvl(case when TS_YEAR_MONTH between '19700101' and '20991231' then TS_YEAR_MONTH - CURRENT_YEAR_MONTH end,999) status
  from(
select 
template_name owner,
table_name,
task TABLESPACE,
task_type partition_name,
substr(task, instr(task,'_',-4,1)+1) TS_YEAR_MONTH,
    TO_CHAR(sysdate,'YYYYMM') current_year_month
from gt_tasks
) a
) b
left outer join dw_table_metadata m
on (b.owner = m.SCHEMA_NAME and b.table_name = m.table_name)
where (status <> '1' and status <> '0') --<-- not in correct TS
) s
where case when current_ts <> ts_var then 'ALERT!!!' else null end  is not null;

---
--30)
--ARE THERE TABLES IN INCORRECT TABLESPACES?
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
select A.* ,  'ALTER TABLE ' || OWNER || '.'
       ||table_name
       ||' MOVE PARTITION '
       ||PARTITION_NAME
       ||' TABLESPACE ' || TS_VAR || '  parallel 32; ' STATEMENT
FROM  
(
select 
s.*,
...
begin
  DW_BIB_MAINT_UTL.FIX_PARTITION_NAMES('STG_GEN', 'HRIS_NEXT_ADDRESS');
